...
public class DummyApiService implements ApiService {
    ...
    @Override
    public void addReunion(Reunion reunion){reunions.add(reunion);}
    ...
    public List<Reunion> filterReunion(String filterPattern) {
        ...
        if (filterPattern == null || filterPattern.length() == 0) {
            ...
            for (Reunion reunion : reunions) {
                ...
                if (reunion.getReunionNumber().toLowerCase().contains(filterLowerCase)) {
                    filteredList.add(reunion);
                }
                if (reunion.getSalle().toLowerCase().contains(filterLowerCase)) {
                    filteredList.add(reunion);
                }
            }
            return filteredList;
